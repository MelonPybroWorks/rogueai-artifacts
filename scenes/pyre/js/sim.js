      ...
      for (let k = 1; k < W - 1; k++) {
        ...
        if (mat === SAND) {
          ...
          if (life[i] > 0) {                                    // burning wood: emits fire, chars down
            ...
            if (m[i - W] === EMPTY && rand() < 0.7) { m[i - W] = FIRE; life[i - W] = this.initLife(FIRE); stamp[i - W] = sid; }
          }
        }
        else if (mat === FIRE) {
          life[i]--;
        ...
        if (mat === FIRE || mat === LAVA) {
          ...
          for (let o = 0; o < 4; o++) {
            const j = i + [W, -W, 1, -1][o];
            const tj = m[j];
            if (tj === WOOD && life[j] === 0 && rand() < (mat === LAVA ? 0.45 : 0.28)) life[j] = 80;
    ...
    for (let y = 1; y < H - 1; y++) {
      ...
      for (let k = 1; k < W - 1; k++) {
        ...
        if (mat === FIRE) {
          ...
          const up = i - W;
          const d = flip ? -1 : 1;
          // an unsupported flame with no fuel anywhere is an ember — it FALLS
          if (!nearFuel && m[i + W] === EMPTY && rand() < 0.7) { this._sw(i, i + W); stamp[i + W] = sid; continue; }
